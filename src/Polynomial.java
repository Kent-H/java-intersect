...
public class Polynomial{
	...
	*****/
	public double[] getQuadraticRoots(){
		double[] results = EMPTY_DOUBLE_ARRAY;
		...
		if (this.getDegree() == 2) {
			...
			if (d > 0) {
				...
				results = new double[] { (0.5 * (-b + e)), (0.5 * (-b - e)) };
			}else if (d == 0) {
	...
	public double[] getCubicRoots(){
		...
		if (this.getDegree() == 3) {
			...
			if (discrim > 0) {
				...
				results = new double[] { (2 * distance * cos - offset), (-distance * (cos + sqrt3 * sin) - offset),
	...
	*****/
	public double[] getQuarticRoots(){
		...
		if (this.getDegree() == 4) {
			...
			double[] resolveRoots = new Polynomial(1, -c2, c3 * c1 - 4 * c0, -c3 * c3 * c0 + 4 * c2 * c0 - c1 * c1)
					.getCubicRoots();
			...
			if (discrim > 0) {
				...
				if (plus >= 0) {
					...
					results = new double[] { (-c3 / 4 + (e + f) / 2), (-c3 / 4 + (e - f) / 2) };
				}
				if (minus >= 0) {
					double f = Math.sqrt(minus);

					results = new double[] { (-c3 / 4 + (f - e) / 2), (-c3 / 4 - (f + e) / 2) };
				}
			}else if (discrim < 0) {
				// no roots
			}else{
				...
				if (t2 >= -Polynomial.TOLERANCE) {
					...
					if (t1 + t2 >= Polynomial.TOLERANCE) {
						...
						results = new double[] { (-c3 / 4 + d / 2), (-c3 / 4 - d / 2) };
					}
					if (t1 - t2 >= Polynomial.TOLERANCE) {
						double d = Math.sqrt(t1 - t2);
